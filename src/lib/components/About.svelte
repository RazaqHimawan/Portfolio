<hgroup class="font-semibold">
  <h1 class="text-3xl">
    Hi, I'm <span class="text-accent dark:text-dark-accent">Razaq Himawan</span>
    👋
  </h1>
  <h2 class="text-xl mt-1">Front-end web developer</h2>
  <p class="text-lg font-normal mt-2">
    I am a front-end developer who is currently based in Bandung, Indonesia.
  </p>
</hgroup>

<div class="my-10">
  <a
    href="mailto:razaqhim@gmail.com"
    class="bg-primary p-4 rounded-md transition-all duration-500 dark:bg-dark-secondary hover:bg-dark-secondary hover:text-dark-text hover:dark:bg-dark-primary
    hover:dark:text-text">Contact Me</a
  >
</div>
